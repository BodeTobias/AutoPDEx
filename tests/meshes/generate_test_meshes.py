"""Generate the VTU meshes used by the transient / least-square FEM tests.

These meshes were previously produced at test time with pygmsh. To avoid a
hard pygmsh dependency for running the test suite, they are generated once with
this script and committed as .vtu files, then loaded with meshio.read in the
tests (same approach as tests/patch_tests/meshes/generate_patch_meshes.py).

Regenerate from the repository root with:
    python tests/meshes/generate_test_meshes.py

Requires the optional dev dependency pygmsh.
"""
...
# Cook's membrane geometry (shared by the Dirichlet and least-square tests).
COOKS_PTS = [[0.0, 0.0], [48.0, 44.0], [48.0, 60.0], [0.0, 44.0]]

# Maze geometry (shared by the forward/backward Euler transient tests).
MAZE_PTS = [[0.94, 8.68], [0.22, 8.68], [0.22, 8.36], [0.64, 8.36], [0.64, 
...
def _polygon_mesh(pts, mesh_size, order):
    with pygmsh.geo.Geometry() as geom:
        geom.add_polygon(pts, mesh_size=mesh_size)
        mesh = geom.generate_mesh(order=order)

    # pygmsh returns uint64 connectivity, which meshio writes as VTU "UInt64"
    # and reads back as float64. Store signed int64 (like the patch meshes) so
    # the arrays load as integer indices.
    cells = [
        meshio.CellBlock(block.type, np.asarray(block.data, dtype=np.int64))
        for block in mesh.cells
    ]
    return meshio.Mesh(points=np.asarray(mesh.points, dtype=float), cells=cells)


def main():
    cases = [
        # filename, polygon, mesh_size, mesh order
        ("cooks_tri_p1.vtu", COOKS_PTS, 50.0, 1),
        ("cooks_tri6_p2.vtu", COOKS_PTS, 50.0, 2),
        ("maze_tri_p1.vtu", MAZE_PTS, 5.0, 1),
    ]
    for name, pts, mesh_size, order in cases:
        mesh = _polygon_mesh(pts, mesh_size, order)
        meshio.write(MESH_DIR / name, mesh, file_format="vtu", binary=False)
        print(f"wrote {name}: {[c.type for c in mesh.cells]}")
