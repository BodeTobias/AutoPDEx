...
def jit_with_docstring(static_argnames=None):
    def decorator(fun):
        ...
        if static_argnames is not None:
            jitted_fun = jax.tree_util.Partial(jax.jit, static_argnames=static_argnames)(fun)
        else:
...
def dict_flatten(arr):
    ...
    if isinstance(arr, dict):
        ...
        if not flat_arrays:
            ...
            return jnp.concatenate(flat_arrays) if isinstance(flat_arrays[0], jnp.ndarray) else np.concatenate(flat_arrays)
...
def reshape_as(flat_array, signature_array):
    ...
    if isinstance(signature_array, dict):
        ...
        for key in sorted(signature_array.keys()):
            ...
            size = sig_value.size if isinstance(sig_value, (np.ndarray, jnp.ndarray)) else 0
            end = start + size
            ...
            if isinstance(sig_value, dict):
                ...
                if isinstance(sig_value, np.ndarray) and not isinstance(reshaped_array, np.ndarray):
                    reshaped_array = np.array(reshaped_array)
                elif isinstance(sig_value, jnp.ndarray) and not isinstance(reshaped_array, jnp.ndarray):
        ...
        if start != flat_array.size:
            raise ValueError("The size of flat_array does not match the total size of signature_array.")
        ...
        if flat_array.size != signature_array.size:
            raise ValueError("The size of flat_array does not match the size of signature_array.")
        reshaped_array = flat_array.reshape(signature_array.shape)
        # Convert to the same type as signature_array if necessary
        if isinstance(signature_array, np.ndarray) and not isinstance(reshaped_array, np.ndarray):
            reshaped_array = np.array(reshaped_array)
        elif isinstance(signature_array, jnp.ndarray) and not isinstance(reshaped_array, jnp.ndarray):
            reshaped_array = jnp.array(reshaped_array)
...
def mask_set(array, selection, values):
  ...
  if isinstance(array, dict):
      ...
      if not isinstance(selection, dict) or not isinstance(values, dict):
          raise TypeError("If 'array' is a dict, 'selection' and 'values' must also be dicts with the same keys.")
      if array.keys() != selection.keys() or array.keys() != values.keys():
          raise ValueError("The keys of 'array', 'selection', and 'values' must match.")
      ...
      if flat_values.size == selected_indices.size:
          ...
          flat_array = flat_array.at[selected_indices].set(flat_values[selected_indices])
      else:
          raise ValueError(
...
def mask_select(array, selection):
    ...
    if isinstance(array, dict):
        ...
        if not isinstance(selection, dict):
            raise TypeError("If 'array' is a dictionary, 'selection' must also be a dictionary with the same keys.")
...
def mask_op(array, selection, values=None, mode='set', ufunc=None):
    ...
    if mode not in mode_methods:
        raise ValueError(f"Invalid mode '{mode}'. Allowed modes are: {list(mode_methods.keys())}")

    if isinstance(array, dict):
        # Ensure selection (and values if needed) are also dicts with the same keys
        if not isinstance(selection, dict):
            raise TypeError("If 'array' is a dict, 'selection' must also be a dict with the same keys.")
        if array.keys() != selection.keys():
            raise ValueError("The keys of 'array' and 'selection' must match.")
        ...
        if mode not in ('get', 'apply') and not isinstance(values, dict):
            raise TypeError("If 'array' is a dict, 'values' must also be a dict with the same keys.")
        ...
        if not (isinstance(selection, np.ndarray) or isinstance(selection, jnp.ndarray)):
...
def jnp_to_tuple(jnp_array):
  ...
  if isinstance(jnp_array, dict):
     return FrozenDict({key: jnp_to_tuple(jnp_array[key]) for key in list(jnp_array.keys())})
  else:
