...
class ModelContext:
  """Argument passed to every high-level model function registered via :meth:`SimState.add_model`.

  A single model function can serve several purposes; the requested one is
  signalled by :attr:`mode` and determines what has to be returned:
  ...
  x_int:
      Coordinates of the current integration point in the reference domain; the
      ansatz callables are evaluated here. Derivatives w.r.t. x_int are computed 
      w.r.t. initial configuration via a custom jvp rule.
  trial_ansatz:
      Dict ``field name -> ansatz(x_int, t)`` returning the trial field. The
      reserved key ``"physical coor"`` maps a reference point to its physical
      position, e.g. ``x = ctx.trial_ansatz["physical coor"](ctx.x_int)``.
  test_ansatz:
      Dict ``field name -> test(x_int)`` used to build the weak-form residual;
      ``None`` in ``"potential"``/``"output"`` mode.
  internal_vars:
      Internal variables at the integration point (e.g. plastic strain), read in
      ``"output"`` mode and forwarded to the local solvers.
  local_solvers:
      Registered local subsystems; can be invoked through :meth:`solve_local`.
  ...
  test_ansatz: Dict[str, Callable] | None = None
  internal_vars: Dict[str, Any] | None = None
  local_solvers: Dict[str, Callable] | None = None
  settings: Dict[str, Any] | None = None
  static_settings: Any = None
  elem_number: Any = None
  int_point_number: Any = None
  set: Any = None
  t: Any = None
  dt: Any = None
  ...
  def __post_init__(self):
    if self.settings is None:
      return
    t = self.settings.get("current time", 0.0)
    last_time = self.settings.get("last time", t)
    object.__setattr__(self, "t", t)
    object.__setattr__(self, "dt", t - last_time)

  def solve_local(self, key: str, inputs: Dict[str, Any] | None = None):
    if self.local_solvers is None or key not in self.local_solvers:
      available = () if self.local_solvers is None else tuple(self.local_solvers.keys())
      raise KeyError(f"Unknown local subsystem '{key}'. Available: {available}")
    if self.internal_vars is None:
      raise ValueError("ModelContext.solve_local(...) requires internal_vars.")
    if self.settings is None:
      raise ValueError("ModelContext.solve_local(...) requires settings.")
    return self.local_solvers[key](
        self.internal_vars,
        self.settings,
        inputs,
        elem_number=self.elem_number,
        int_point_number=self.int_point_number,
        set=self.set,
    )


def _scalar_context_result(value, mode, fun_name):
  if isinstance(value, dict):
    raise ValueError(f"{fun_name} must return a scalar in '{mode}' mode, not a dict.")
  value = jnp.asarray(value)
  if value.shape != ():
    raise ValueError(f"{fun_name} must return a scalar in '{mode}' mode, got shape {value.shape}.")
  return value


def _internal_variable_context_result(values, internal_variable_names, fun_name):
  if not isinstance(values, dict):
    raise ValueError(f"{fun_name} must return an internal-variable dict in 'internal variables' mode.")
  expected = tuple(internal_variable_names)
  if set(values.keys()) != set(expected):
    raise ValueError(
        f"{fun_name} must return internal variables {expected}, got {tuple(values.keys())}."
    )
  return values
...
def hyperelastic_steady_state_weak(
  ...
  def pde_fun(
    ...
    if volume_load_fun == None:
      ...
      PI_ext = volume_load_fun(x) @ virt_u
...
def mixed_reference_domain_potential(
  ...
  ref_int_coor = jnp.asarray(ref_int_coor)
  ref_int_weights = jnp.asarray(ref_int_weights)

  def user_potential(fI, xI, elem_number, settings, static_settings, set):
    xi_is_dict = isinstance(xI, dict)
    if xi_is_dict:
      x_map = xI[mapping_key] if mapping_key in xI else xI[field_keys[0]]
      n_dim = x_map.shape[-1]
      trial_ansatz = {
          key: (lambda x, key=key: ansatz_fun[key](x, xI[key], fI[key], settings, True, n_dim))
          for key in field_keys
      }
      mapping = lambda x: ansatz_fun[mapping_key](x, x_map, x_map, settings, False, n_dim)
    else:
      n_dim = xI.shape[-1]
      trial_ansatz = {
          key: (lambda x, key=key: ansatz_fun[key](x, xI, fI[key], settings, True, n_dim))
          for key in field_keys
      }
      mapping = lambda x: ansatz_fun[mapping_key](x, xI, xI, settings, False, n_dim)
    ...
    # # foriloop
    # body_fun = lambda i, total: total + functional(ref_int_coor[i], ref_int_weights[i])
    # return jax.lax.fori_loop(0, ref_int_coor.shape[0], body_fun, 0.0, unroll=True)

    # for ref_int_i in range(ref_int_coor.shape[0]):
    #   if ref_int_i == 0:
    #     total = functional(ref_int_coor[ref_int_i], ref_int_weights[ref_int_i])
    #   else:
    #     total += functional(ref_int_coor[ref_int_i], ref_int_weights[ref_int_i])
    # return total
  ...
  def user_residual(fI, xI, elem_number, settings, static_settings, set):
    ...
    def integrated_weak_form(test_fI):
      ...
      if xi_is_dict:
        ...
        test_ansatz = {
            key: (lambda x, key=key: ansatz_fun[key](x, xI[key], test_fI[key], settings, True, n_dim))
            for key in field_keys
        }
        ...
        test_ansatz = {
            key: (lambda x, key=key: ansatz_fun[key](x, xI, test_fI[key], settings, True, n_dim))
            for key in field_keys
        }
        mapping = lambda x: ansatz_fun[mapping_key](x, xI, xI, settings, False, n_dim)
    ...
    trial_ansatz = {
        key: (
            lambda x, key=key: ansatz_fun[key](
                x, xI[key], fI[key], settings, True, n_dim - 1
            )
        )
        for key in field_keys
    }
  ...
  def user_residual(fI, xI, elem_number, settings, static_settings, set):
    ...
    def integrated_weak_form(test_fI):
      ...
      test_ansatz = {
          key: (
              lambda x, key=key: ansatz_fun[key](
                  x, xI[key], test_fI[key], settings, True, n_dim - 1
              )
          )
          for key in field_keys
      }
  ...
  def user_element(fI, xI, elem_number, settings, static_settings, mode, set):
    ...
    def residual_and_tangent(x_int, w_int):
      ...
      def integrated_weak_form(test_fI, trial_fI):
        # Define trial and test function ansatz
        trial_ansatz = lambda x: ansatz_fun(
            x, xI, trial_fI, settings, True, n_dim - 1
        )
        test_ansatz = lambda x: ansatz_fun(
            x, xI, test_fI, settings, True, n_dim - 1
        )
...
def mixed_reference_surface_residual_time(integrand_fun, ansatz_fun, ref_int_coor, ref_int_weights, mapping_key):
  """
  Constructs a multi-field 'user residual' for time-dependent surface integrals.
  ...
  Args:
    ...
    ansatz_fun (dict of callables): Each function must have signature:
        f(x, t, xI, fI, settings, is_trial, n_dim)
    ref_int_coor (jnp.ndarray): Reference integration coordinates (on reference surface).
    ref_int_weights (jnp.ndarray): Integration weights.
    mapping_key (str): Key used for the isoparametric surface mapping.

  Returns:
    A function with signature:
        user_residual(fI, xI, elem_number, settings, static_settings, set)
    that computes the element surface residual.
  """
  keys = ansatz_fun.keys()
  ref_int_coor = jnp.asarray(ref_int_coor)
  ref_int_weights = jnp.asarray(ref_int_weights)
  ...
  def user_residual(fI, xI, elem_number, settings, static_settings, set):
    n_dim = xI[mapping_key].shape[-1]

    def integrated_weak_form(test_fI):
      # Define trial and test ansatz functions for each field
      trial_ansatz = {}
      test_ansatz = {}

      for key in keys:
        trial_ansatz[key] = lambda x, t, key=key: ansatz_fun[key](x, xI[key], fI(t)[key], settings, True, n_dim - 1)
        test_ansatz[key] = lambda x, key=key: ansatz_fun[key](x, xI[key], test_fI[key], settings, True, n_dim - 1)

      # Isoparametric mapping
      def mapping(x):
        return ansatz_fun[mapping_key](x, xI[mapping_key], xI[mapping_key], settings, False, n_dim - 1)
      ...
      def functional(x_int, w_int, int_num):
        ...
        match n_dim:
          ...
          case 3:
            ...
            scaling = jnp.linalg.norm(jnp.cross(G_1, G_2))
          case _:
            raise ValueError("Surface elements only supported in 2D or 3D.")
    ...
    # Derivative of integrated weak form wrt. test nodal values
    test_fI = utility.dict_zeros_like(fI(0.))
    residual = jax.jacrev(integrated_weak_form)(test_fI)
    return residual
...
## Functions for high-level-API
def _select_internal_variables(settings, set, elem_number, int_num, internal_variable_names):
    if len(internal_variable_names) == 0:
        return {}
    store = settings.get("internal variables", settings.get("internal variables n", None))
    if store is None:
        raise KeyError("settings must contain 'internal variables' for models using internal_vars.")
    block = store[set] if isinstance(store, (tuple, list)) else store
    return {
        name: block[name][elem_number, int_num]
        for name in internal_variable_names
    }

def _make_caller_adapter(fun: Callable) -> Callable:
    """Wraps old-style (6-arg) ansatz callers to the new 8-arg signature."""
    params = signature(fun).parameters
    if ("elem_number" in params) and ("set" in params) and ("xI_dict" in params or "xI" in params):
        return fun
    def call(x, xI_dict, fI_key, settings, overwrite_diff, n_dim, elem_number, set):
        return fun(x, xI_dict, fI_key, settings, overwrite_diff, n_dim)
    return call

def _call_high_level_weak_form(
    integrand_fun,
    x_int,
    trial_ansatz,
    test_ansatz,
    ...
    ctx = ModelContext(
        ...
        test_ansatz=test_ansatz,
        internal_vars=internal_vars,
        local_solvers=static_settings.get("local subsystem solvers", None),
        settings=settings,
    ...
    return _scalar_context_result(integrand_fun(ctx), "weak form", "integrand_fun")

def _call_high_level_internal_variable_update(
    update_fun,
    x_int,
    trial_ansatz,
    internal_vars,
    ...
    return _internal_variable_context_result(
        update_fun(ctx), internal_variable_names, "update_fun"
    )

def _call_high_level_potential(
    integrand_fun,
    x_int,
    trial_ansatz,
    internal_vars,
    ...
    return _scalar_context_result(integrand_fun(ctx), "potential", "integrand_fun")

def _build_element_model(
    mode: str,
    integrand_fun: Callable,
    ansatz_fun: Dict[str, Callable],
    ref_int_coor: jnp.ndarray | np.array,
    ref_int_weights: jnp.ndarray | np.array,
    internal_variable_names=(),
):
    """Shared implementation core for weak_form and potential element models.

    Parameters
    ----------
    mode:
        'weak form' — residual via jacrev wrt test DOFs.
        'potential' — residual via grad wrt trial DOFs (no test ansatz).
    """
    ref_int_coor = jnp.asarray(ref_int_coor)
    ref_int_weights = jnp.asarray(ref_int_weights)
    domain_dim = ref_int_coor.shape[1] if ref_int_coor.ndim >= 2 else 1
    keys = tuple(ansatz_fun.keys())
    internal_variable_names = tuple(internal_variable_names)

    if mode not in ("weak form", "potential"):
        raise ValueError(f"mode must be 'weak form' or 'potential', got '{mode}'.")
    if len(signature(integrand_fun).parameters) != 1:
        raise ValueError(
            "integrand_fun must take one ModelContext argument in the high-level interface."
        )

    callers = {k: _make_caller_adapter(ansatz_fun[k]) for k in keys}
    field_keys = tuple(k for k in keys if k != "physical coor")

    def user_residual(fI, xI, elem_number, settings, static_settings, set):

        def mapping(x):
          return callers['physical coor'](x, xI, xI, settings, False, domain_dim, elem_number, set)
        ...
        def _scaling(x_int):
            ...
            if domain_dim == 2 and problem_dim == 3:
                return jnp.linalg.norm(jnp.cross(jacobian[:, 0], jacobian[:, 1]))
            if domain_dim == 1 and problem_dim in (2, 3):
                return jnp.linalg.norm(jacobian)
            raise ValueError(
                f"Unsupported combination of domain and problem dimension: "
                f"{domain_dim}D domain in {problem_dim}D space."
            )

        if mode == "weak form":
            def integrated_form(test_fI):
                trial_ansatz = {
                    key: (lambda x, t, key=key:
                        utility.scale_dof_value(
                            settings, key,
                            callers[key](x, xI, fI(t)[key], settings, True, domain_dim, elem_number, set)
                        ))
                    for key in field_keys
                }
                trial_ansatz['physical coor'] = mapping
                test_ansatz = {
                    key: (lambda x, key=key:
                        utility.scale_dof_value(
                            settings, key,
                            callers[key](x, xI, test_fI[key], settings, True, domain_dim, elem_number, set)
                        ))
                    for key in field_keys
                }

                def functional(x_int, w_int, int_num):
                    internal_vars = _select_internal_variables(
                        settings, set, elem_number, int_num, internal_variable_names
                    )
                    val = _call_high_level_weak_form(
                        integrand_fun, x_int, trial_ansatz, test_ansatz,
                        internal_vars, settings, static_settings, elem_number, int_num, set,
                    )
                    return val * w_int * _scaling(x_int)
            ...
            return jax.jacrev(integrated_form)(utility.dict_zeros_like(fI(0.)))

        else:  # "potential"
            def integrated_form(trial_fI):
                trial_ansatz = {
                    key: (lambda x, t, key=key:
                        utility.scale_dof_value(
                            settings, key,
                            callers[key](x, xI, trial_fI[key], settings, True, domain_dim, elem_number, set)
                        ))
                    for key in field_keys
                }
                trial_ansatz['physical coor'] = mapping
                ...
                def functional(x_int, w_int, int_num):
                    ...
                    val = _call_high_level_potential(
                        integrand_fun, x_int, trial_ansatz,
                        internal_vars, settings, static_settings, elem_number, int_num, set,
                    )
                    return val * w_int * _scaling(x_int)
            ...
            return jax.grad(integrated_form)(fI(0.))
...
def weak_form(
    ...
    return _build_element_model(
        "weak form", integrand_fun, ansatz_fun, ref_int_coor, ref_int_weights, internal_variable_names
    )
...
def potential(
    ...
    return _build_element_model(
        "potential", integrand_fun, ansatz_fun, ref_int_coor, ref_int_weights, internal_variable_names
    )

def internal_variable_updates(
    ...
    if ref_int_coor.ndim >= 2:
      domain_dim = ref_int_coor.shape[1]
    else:
      domain_dim = 1
    ...
    if update_fun is not None and len(signature(update_fun).parameters) != 1:
        raise ValueError(
            "update_fun must take one ModelContext argument in the high-level interface."
        )
    ...
    def int_var_fun(fI, xI, elem_number, settings, static_settings, set):
        ...
        for key in field_keys:
            trial_ansatz[key] = (lambda x, t, key=key:
                utility.scale_dof_value(
                    settings, key,
                    callers[key](x, xI, fI(t)[key], settings, True, domain_dim, elem_number, set)
                )
        ...
        trial_ansatz['physical coor'] = (lambda x:
                callers['physical coor'](x, xI, xI, settings, False, domain_dim, elem_number, set)
            )
        ...
        def local_fun(x_int, int_num):
            ...
            return _call_high_level_internal_variable_update(
                update_fun,
                x_int,
                trial_ansatz,
                internal_vars,
        ...
        int_nums = jnp.arange(ref_int_coor.shape[0])
        return jax.vmap(local_fun, (0, 0), 0)(ref_int_coor, int_nums)
...
def aug_lag_potential(lag, constr, eps = 1.0):
  """
  ...
  f1 = constr * (lag + 0.5 * eps * constr)
  f2 = -lag**2 / (2 * eps)
  return jnp.where(aug_lag <= 0.0, f1, f2)
