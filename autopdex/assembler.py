...
def _get_indices(connectivity, dofs):
    ...
    if isinstance(dofs, dict):
      field_keys = list(dofs.keys())  # Verwenden Sie die natürliche Reihenfolge der Schlüssel
      ...
      for field_i in field_keys:
          for field_j in field_keys:
              def one_elem_indices(elem_idx):
                  ...
                  dof_indices_i = field_offset_i + conn_i[:, None] * dofs_per_node_i + dof_local_i
                  ...
                  dof_indices_j = field_offset_j + conn_j[:, None] * dofs_per_node_j + dof_local_j
...
def _get_element_quantities(dofs, settings, static_settings, set):
    ...
    if isinstance(dofs, dict):
        assert isinstance(x_nodes, dict), "If 'dofs' is a dict, 'settings['node coordinates']' must also be a dict."

        connectivity_dict = static_settings['connectivity'][set]
        assert isinstance(connectivity_dict, (dict, FrozenDict)), \
...
def _get_tangent_diagonal(tangent_contributions, connectivity, dofs):
    ...
    num_dofs = dofs.size if not isinstance(dofs, dict) else sum(v.size for v in dofs.values())
    ...
    if isinstance(dofs, dict):
        ...
        for key in field_keys:
            ...
            diagonal_contributions = jnp.diagonal(tc, axis1=1, axis2=2)  # Shape: (n_elems, element_dofs)
            ...
            global_dofs = dof_indices.reshape(n_elems, element_dofs)  # Shape: (n_elems, element_dofs)
        ...
        # Compute global DOF indices
        dof_local = jnp.arange(dofs_per_node)
        dof_indices = conn[:, :, None] * dofs_per_node + dof_local[None, None, :]  # Shape: (n_elems, nodes_per_element, dofs_per_node)
        global_dofs = dof_indices.reshape(n_elems, element_dofs)  # Shape: (n_elems, element_dofs)
...
def _get_residual(residual_contributions, connectivity, dofs):
    ...
    if isinstance(dofs, dict):
        ...
        for key in field_keys:
            ...
            rc = residual_contributions[key]  # Shape: (n_elems, nodes_per_element, dofs_per_node)
        ...
        rc = residual_contributions.reshape(n_elems, element_dofs)  # Shape: (n_elems, element_dofs)
...
def assemble_residual(dofs, settings, static_settings):
  ...
  for set in range(num_sets):
    ...
    if isinstance(add, dict):
      integrated_residual = {key: integrated_residual[key] + add[key] for key in add}
    else:
...
def assemble_tangent(dofs, settings, static_settings):
  ...
  match sparsity_pattern:
    ...
    case 'diagonal':
      ...
      diag = dict_flatten(assemble_tangent_diagonal(dofs, settings, static_settings))
...
def dense_assemble_tangent(dofs, settings, static_settings, set):
  ...
  """
  assert isinstance(dofs, jnp.ndarray), "Dense mode of tangent assembly does currently not support dofs as dicts."

  size = dict_flatten(dofs).size
  hessian = jax.hessian(dense_integrate_functional)(dofs, settings, static_settings, set)
...
def sparse_integrate_functional(dofs, settings, static_settings, set):
  ...
  Returns:
    float: The integrated functional value.
  """
  assert isinstance(dofs, jnp.ndarray), "Variational schemes do currently not support dofs as dicts."

  _, local_dofs, _, _, _ = _get_element_quantities(dofs, settings, static_settings, set)
  x_int = settings['integration coordinates'][set]
  w_int = settings['integration weights'][set]
  int_point_numbers = jnp.arange(0, x_int.shape[0], 1)
...
def sparse_assemble_residual(dofs, settings, static_settings, set):
  ...
  _, local_dofs, _, _, connectivity = _get_element_quantities(dofs, settings, static_settings, set)
  ...
  if variational_scheme == 'least square pde loss' or variational_scheme == 'least square function approximation':
    residual_at_int_point_vj = jax.vmap(jax.jacrev(variational_schemes.functional_at_int_point, argnums=3), (0,0,0,0,None,None,None), 0)
...
def user_potential_integrate_functional(dofs, settings, static_settings, set):
  ...
  """
  model_fun, local_dofs, local_node_coor, elem_numbers, _ = _get_element_quantities(dofs, settings, static_settings, set) 
...
def user_potential_assemble_residual(dofs, settings, static_settings, set):
  ...
  Returns:
    jnp.ndarray: The assembled residual.
  """
  model_fun, local_dofs, local_node_coor, elem_numbers, connectivity = _get_element_quantities(dofs, settings, static_settings, set) 

  residual_contributions = jax.vmap(jax.jacrev(model_fun), (0, 0, 0, None, None, None), (0))(local_dofs, local_node_coor, elem_numbers, settings, static_settings, set)
...
def user_potential_assemble_tangent_diagonal(dofs, settings, static_settings, set):
  ...
  tangent_contributions = jax.vmap(jax.jacfwd(jax.jacrev(model_fun)), (0, 0, 0, None, None, None), (0))(local_dofs, local_node_coor, elem_numbers, settings, static_settings, set)
...
def user_residual_assemble_residual(dofs, settings, static_settings, set):
  ...
  residual_contributions = jax.vmap(model_fun, (0, 0, 0, None, None, None), (0))(local_dofs, local_node_coor, elem_numbers, settings, static_settings, set)
...
def user_residual_assemble_tangent_diagonal(dofs, settings, static_settings, set):
  ...
  tangent_contributions = jax.vmap(jax.jacfwd(model_fun), (0, 0, 0, None, None, None), (0))(local_dofs, local_node_coor, elem_numbers, settings, static_settings, set)
...
def user_residual_assemble_tangent(dofs, settings, static_settings, set):
  ...
  tangent_contributions = jax.vmap(jax.jacfwd(model_fun), (0, 0, 0, None, None, None), (0)
                                   )(local_dofs, local_node_coor, elem_numbers, settings, static_settings, set)
...
def user_element_assemble_residual(dofs, settings, static_settings, set):
  ...
  residual_contributions = jax.vmap(model_fun, (0, 0, 0, None, None, None, None), (0))(local_dofs, local_node_coor, elem_numbers, settings, static_settings, 'residual', set)
...
def user_element_assemble_tangent_diagonal(dofs, settings, static_settings, set):
  ...
  tangent_contributions = jax.vmap(model_fun, (0, 0, 0, None, None, None, None), (0))(local_dofs, local_node_coor, elem_numbers, settings, static_settings, 'tangent', set)
