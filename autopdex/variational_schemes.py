...
def functional_at_int_point(x_i, w_i, int_point_number, local_dofs, settings, static_settings, set):
  """
  ...
  match algorithm:
    case 'least square function approximation':
      return least_square_function_approximation(x_i, w_i, int_point_number, local_dofs, settings, static_settings, set)
    case 'least square pde loss':
      return least_square_pde_loss(x_i, w_i, int_point_number, local_dofs, settings, static_settings, set)
    case _:
      assert False, 'variational scheme is not defined or not available with this settings.'

@jit_with_docstring(static_argnames=['static_settings', 'set'])
def direct_residual_at_int_point(x_i, w_i, int_point_number, local_dofs, settings, static_settings, set):
  """
  ...
  match algorithm:
    case 'strong form galerkin':
      return strong_form_galerkin(x_i, w_i, int_point_number, local_dofs, settings, static_settings, set)
    case _:
      assert False, 'variational scheme is not defined or not available with this settings.'

@jit_with_docstring(static_argnames=['static_settings', 'set'])
def residual_from_deriv_at_int_point(x_i, w_i, int_point_number, local_dofs, local_virt_dofs, settings, static_settings, set):
  """
  ...
  match algorithm:
    case 'weak form galerkin':
      return weak_form_galerkin(x_i, w_i, int_point_number, local_dofs, local_virt_dofs, settings, static_settings, set)
    case _:
      assert False, 'variational scheme is not defined or not available with this settings.'

@jit_with_docstring(static_argnames=['static_settings', 'set'])
def least_square_pde_loss(x_i, w_i, int_point_number, local_dofs, settings, static_settings, set):
  """
  ...
  def approximate_solution(x):
    ansatz = ss.solution_structure(x, int_point_number, local_dofs, settings, static_settings, set)
  ...
  pde = static_settings['model'][set](x_i, approximate_solution, settings, static_settings, int_point_number, set)

  return w_i * jnp.dot(pde, pde) / 2

@jit_with_docstring(static_argnames=['static_settings', 'set'])
def weak_form_galerkin(x_i, w_i, int_point_number, local_dofs, local_virt_dofs, settings, static_settings, set):
  """
  ...
  def weak_form(virtual_dofs):
    def test_function(x):
      if local_dofs.shape[1] == 1:
        return ss.solution_structure(x, int_point_number, virtual_dofs, settings, static_settings, set)[0]
      else:
        return ss.solution_structure(x, int_point_number, virtual_dofs, settings, static_settings, set)
    def trial_function(x):
      if local_dofs.shape[1] == 1:
        return ss.solution_structure(x, int_point_number, local_dofs, settings, static_settings, set)[0]
      else:
...
@jit_with_docstring(static_argnames=['static_settings', 'set'])
def strong_form_galerkin(x_i, w_i, int_point_number, local_dofs, settings, static_settings, set):
  """
  ...
  if shp_fun_mode == 'compiled':
    ...
    test_functions = jax.jacrev(spaces.solution_space, argnums=2)(x_i, int_point_number, local_dofs, settings, static_settings, set)[0,:,0]
  else:
...
@jit_with_docstring(static_argnames=['static_settings', 'set'])
def least_square_function_approximation(x_i, w_i, int_point_number, local_dofs, settings, static_settings, set):  
  """
