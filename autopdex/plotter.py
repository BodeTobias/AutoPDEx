...
def pv_plot(x_vis, data, export_vtk=False, show=True, groupname='group', filename='frame', fieldname='data', scale_range=1.0, data_on_z=True):
  ...
  if x_vis.shape[1] == 2:
      ...
      scaled_data = scale_range * (x_range / data_range) * data if data_range > 1e-8 else data
      positions = np.column_stack((x_vis[:, 0], x_vis[:, 1], scaled_data if data_on_z else np.zeros_like(scaled_data)))
  else:
...
def pv_function_plot(x_vis, fun, scale_range=1.0, data_on_z=True, isosurf=False, n_surfaces=20):
  """
  ...
  if isosurf:
      ...
      pv_plot(x_vis, data, export_vtk=False, show=True, scale_range=scale_range, data_on_z=data_on_z)
