...
def structured_mesh(n_elements, vertices, element_type, order=1):
    """
    Generates a structured 2D or 3D mesh over a quadrilateral or hexahedral domain,
    with optional subdivision into simplex (triangular/tetrahedral) elements.

    For 2D:
      - vertices should be a 4x2 array (in anti-clockwise order).
      - element_type can be 'quad' for quadrilaterals or 'tri' for triangles.
      - The mapping from a reference square ([-1,1]x[-1,1]) to the physical domain
        is done via bilinear interpolation.
    
    For 3D:
      - vertices should be an 8x3 array (with the ordering corresponding to a
        standard hexahedron, e.g., starting with (-1,-1,-1) and proceeding in an
        anti-clockwise fashion on the bottom face, then the top face).
      - element_type can be 'brick' for hexahedral elements or 'tet' for tetrahedral elements.
      - The mapping from a reference cube ([-1,1]^3) to the physical domain is done
        via trilinear interpolation.
    
    Note:
      Currently only linear (order==1) elements are supported.
      Simplex elements are obtained by subdividing each quadrilateral/brick element.
    
    Args:
        n_elements (tuple of int): For 2D, a tuple (nx, ny) specifying the number of
            elements along the x- and y- directions. For 3D, a tuple (nx, ny, nz).
        vertices (array-like): For 2D a 4x2 array; for 3D an 8x3 array defining the
            coordinates of the domain's corner points.
        element_type (str): For 2D, either 'quad' or 'tri'; for 3D, either 'brick' or 'tet'.
        order (int, optional): The polynomial order of the elements. Only order==1 is implemented.
    
    Returns:
        coords (jnp.ndarray): An array of node coordinates (shape: ((n+1)*... x dim)).
        elements (jnp.ndarray): An array of element connectivity. For quadrilaterals/brick elements,
            each row lists the indices of the nodes forming the element. For simplex elements,
            each row lists the indices for the triangle (3) or tetrahedron (4).
    """
    ...
    # ----- 3D Mesh Generation -----
        ...
        def trilinear_interpolate(pt):
            s, t, u = pt
            return (
                (1 - s) * (1 - t) * (1 - u) * vertices[0] +
                (1 + s) * (1 - t) * (1 - u) * vertices[1] +
                (1 + s) * (1 + t) * (1 - u) * vertices[2] +
                (1 - s) * (1 + t) * (1 - u) * vertices[3] +
                (1 - s) * (1 - t) * (1 + u) * vertices[4] +
                (1 + s) * (1 - t) * (1 + u) * vertices[5] +
                (1 + s) * (1 + t) * (1 + u) * vertices[6] +
                (1 - s) * (1 + t) * (1 + u) * vertices[7]
            ) / 8
