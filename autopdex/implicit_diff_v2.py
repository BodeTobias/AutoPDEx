...
"""
These functions are a modification of the functions in jax.lax.custom_root for
external solve functions with a jax.experimental.sparse.BCOO matrix as an argument instead of a matvec 
function and derivatives w.r.t. constraints are taken into account.
With the wrapper custom_root, root solvers can be made differentiable both in forward or reverse mode of arbitrary order.
Mixing the differentiation mode is possible in case the linear solver is defined via jax.lax.custom_linear_solve.
"""
...
from jax.tree_util import (tree_flatten, treedef_children, tree_leaves,
                           tree_unflatten, treedef_tuple, tree_map)
...
from autopdex.utility import dict_flatten, dict_zeros_like, reshape_as, mask_op
...
@api_boundary
def custom_root(f, initial_guess, solve, tangent_solve, free_dofs=None, dirichlet_conditions=None, has_aux=False):
  ...
  Args:
    ...
    free_dofs: a tree of booleans with the same structure as initial_guess
