...
pts = jnp.asarray([[0.94, 8.68], [0.22, 8.68], [0.22, 8.36], [0.64, 8.36], [0.64, 
  ...
  8.24], [0.94, 8.24]])
example_psdf = lambda x: geo.psdf_polygon(x, pts)

# Example of scattered data in 2d
min, max = (jnp.min(pts, axis=0), jnp.max(pts, axis=0))
spacing = (max[0] - min[0]) / 1000
x_vis, _, _ = seeder.regular_in_psdf(example_psdf, min, max, spacing)
plotter.pv_function_plot(x_vis, example_psdf, scale_range=0.1)
